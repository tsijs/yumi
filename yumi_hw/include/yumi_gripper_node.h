...
protected:
  bool internalCB(industrial::simple_message::SimpleMessage &in) {
    ...
    if (industrial::simple_message::CommTypes::SERVICE_REQUEST ==
      ...
      reply.init(MSG_TYPE_GRIPPER_STATE,
                 industrial::simple_message::CommTypes::SERVICE_REPLY,
                 ret ? industrial::simple_message::ReplyTypes::SUCCESS
...
public:
  ...
  ~YumiGripperStateInterface() { stopThreads(); }
  ...
  void startThreads() {
    if (!stopComm_) {
      boost::thread(boost::bind(
          &YumiGripperStateInterface::RapidCommThreadCallback, this));
    }
  }
  ...
  void setGripperEfforts(float left, float right) {
    ...
    grasp_msg.init(MSG_TYPE_GRIPPER_COMMAND,
  ...
  bool init(std::string ip = "", int port = DEFAULT_STATE_PORT,
...
public:
  YumiGripperNode() {
    ...
    // read in parameters
    nh_.param<std::string>("joint_state_topic", gripper_state_topic,
                           "gripper_states");
    nh_.param<std::string>("grasp_request_topic", grasp_request_topic,
                           "do_grasp");
    nh_.param<std::string>("grasp_release_topic", grasp_release_topic,
                           "release_grasp");
    ...
    heartbeat_ = nh_.createTimer(ros::Duration(js_rate),
                                 &YumiGripperNode::publishState, this);
    request_grasp_ = nh_.advertiseService(
        grasp_request_topic, &YumiGripperNode::request_grasp, this);
    request_release_ = nh_.advertiseService(
        grasp_release_topic, &YumiGripperNode::request_release, this);
    ;
    gripper_status_publisher_ =
        ros::NodeHandle().advertise<sensor_msgs::JointState>(
            gripper_state_topic, 10);
    ...
    left_gripper_cmd_sub =
        nh_.subscribe("/yumi/gripper_l_effort_cmd", 100,
                      &YumiGripperNode::leftGripperCmdCallback, this);
    right_gripper_cmd_sub =
        nh_.subscribe("/yumi/gripper_r_effort_cmd", 100,
                      &YumiGripperNode::rightGripperCmdCallback, this);
  }
...
private:
  ...
  bool request_grasp(yumi_msgs::YumiGrasp::Request &req,
                     yumi_msgs::YumiGrasp::Response &res) {
  ...
  bool request_release(yumi_msgs::YumiGrasp::Request &req,
                       yumi_msgs::YumiGrasp::Response &res) {
    float left = 0, right = 0;
    if (req.gripper_id == LEFT_GRIPPER) {
      left = -default_force;
