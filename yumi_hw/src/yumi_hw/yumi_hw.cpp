...
void YumiHW::create(std::string name, std::string urdf_string) {
  ROS_INFO_STREAM("Creating a Yumi HW interface for: "
                  << name << " with " << n_joints_ << " joints");
  ...
  const urdf::Model *const urdf_model_ptr =
      urdf_model_.initString(urdf_string_) ? &urdf_model_ : NULL;
  registerInterfaces(urdf_model_ptr, transmissions_);

  ROS_INFO(
      "Succesfully created an abstract Yumi with interfaces to ROS control");
}
...
void YumiHW::registerInterfaces(
  ...
  for (int j = 0; j < n_joints_; j++) {
    ...
    if (transmissions[j].joints_.size() == 0) {
      ...
      ROS_WARN_STREAM("Transmission " << transmissions[j].name_
                                      << " has more than one joint, and they "
                                         "can't be controlled simultaneously"
                                      << std::endl);
      continue;
    }
    ...
    joint_handle_effort =
    hardware_interface::JointHandle(state_interface_.getHandle(joint_names_[j]),
    ...
    registerJointLimits(joint_names_[j],
                        ...
                        urdf_model, &joint_lower_limits_[j],
...
// effort limit.
void YumiHW::registerJointLimits(
  ...
  for (int j = 0; j < n_joints_; ++j) {
    ...
    for (; it != transmissions.end(); ++it) {
      ...
      if (joint_names_[j].compare(it->joints_[0].name_) == 0) {
        ...
        // std::cout << "Found a match for transmission " << it->name_ <<
        // std::endl;
      }
    }
  }
...
bool YumiHW::canSwitch(
  ...
  for (std::list<hardware_interface::ControllerInfo>::const_iterator it =
           start_list.begin();
       it != start_list.end(); ++it) {
    if (it->type.compare(
            std::string("hardware_interface::VelocityJointInterface")) == 0) {
      desired_strategies.push_back(JOINT_VELOCITY);
      ROS_WARN("Uncharted teritories here: switching to VelocityInterface\n");
    } else if (it->type.compare(std::string(
                   "hardware_interface::PositionJointInterface")) == 0) {
      ...
      // Debug
      // std::cout << "This controller does not use any command interface, so it
      // is only sensing, no problem" << std::endl;
    }
  }

  if (desired_strategies.size() > 1) {
    ROS_ERROR("Only a single controller can be active at a time. Choose one "
...
void YumiHW::doSwitch(
  ...
  for (std::list<hardware_interface::ControllerInfo>::const_iterator it =
    ...
    for (int i = 0; i < it->claimed_resources.size(); i++) {
      if (it->claimed_resources[i].hardware_interface.compare(
              std::string("hardware_interface::PositionJointInterface")) == 0) {
        ...
        wantsPosition = true;
                     ...
                     std::string(
                         "hardware_interface::VelocityJointInterface")) == 0) {
        ROS_INFO("Request to switch to "
        ...
        ROS_INFO("Controller of type %s, requested interface of type %s. "
                 "Impossible, sorry.\n",
                 it->type.c_str(),
                 it->claimed_resources[i].hardware_interface.c_str());
      }
    }
#else
    // indigo and below
    if (it->hardware_interface.compare(
            std::string("hardware_interface::PositionJointInterface")) == 0) {
      ROS_INFO("Request to switch to "
               "hardware_interface::PositionJointInterface (JOINT_POSITION)");
      desired_strategy = JOINT_POSITION;
      break;
    } else if (it->hardware_interface.compare(std::string(
                   "hardware_interface::VelocityJointInterface")) == 0) {
      ROS_INFO("Request to switch to "
               "hardware_interface::VelocityJointInterface (JOINT_VELOCITY)");
  ...
  if (wantsPosition && wantsVelocity) {
    ROS_ERROR("Cannot have both position and velocity interface. Will assume "
              "Velocity. Beware!");
  }
  ...
  for (int j = 0; j < n_joints_; ++j) {
    ...
    /// call setCommand once so that the JointLimitsInterface receive the
    /// correct value on their getCommand()!
    try {
      position_interface_.getHandle(joint_names_[j])
          .setCommand(joint_position_command_[j]);
    } catch (const hardware_interface::HardwareInterfaceException &) {
    }

    // try{
    // effort_interface_.getHandle(joint_names_[j]).setCommand(joint_effort_command_[j]);
    // }
    // catch(const hardware_interface::HardwareInterfaceException&){}
    try {
      velocity_interface_.getHandle(joint_names_[j])
          .setCommand(joint_velocity_command_[j]);
    } catch (const hardware_interface::HardwareInterfaceException &) {
    }
  ...
  if (desired_strategy == getControlStrategy()) {
    std::cout << "The ControlStrategy didn't change, it is already: "
              << getControlStrategy() << std::endl;
  } else {
