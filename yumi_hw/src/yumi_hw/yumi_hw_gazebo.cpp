...
public:
  ...
  virtual void Load(gazebo::physics::ModelPtr parent, sdf::ElementPtr sdf) {
    ...
    if (!ros::isInitialized()) {
      ROS_FATAL_STREAM_NAMED("yumi_hw", "A ROS node for Gazebo has not been "
    ...
    if (sdf_->HasElement("controlPeriod")) {
      ...
      ROS_DEBUG_STREAM_NAMED("yumi_hw", "Control period not found in URDF/SDF, "
                                        "defaulting to Gazebo period of "
                                            << control_period_);
    }

    // Get parameters/settings for controllers from ROS param server
    model_nh_ = ros::NodeHandle(robot_namespace_);
    ROS_INFO_NAMED("yumi_hw", "Starting yumi_hw plugin in namespace: %s",
    ...
    // Load the YumiHWsim abstraction to interface the controllers with the
    // gazebo model
  ...
  std::string getURDF(std::string param_name) const {
    ...
    while (urdf_string.empty()) {
      ...
      if (model_nh_.searchParam(param_name, search_param_name)) {
        ROS_INFO_ONCE_NAMED("YumiHWsim",
                            "YumiHWsim plugin is waiting for model"
    ...
    ROS_DEBUG_STREAM_NAMED("YumiHWsim",
